...
class PyTorchModelTrainer(PyTorchTrainerInterface):
    ...
    def fit(self, data_dictionary: Dict[str, pd.DataFrame], splits: List[str]):
        ...
        data_loaders_dictionary = self.create_dfata_loaders_dictionary(data_dictionary, splits)
...
class PyTorchPatchTSTTrainer(PyTorchModelTrainer):
    def __init__(
        self,
        model: nn.Module,
        optimizer: Optimizer,
    ...
    def create_data_loaders_dictionary(
        ...
        """Create data loaders with proper windowing for PatchTST"""
        data_loader_dictionary = {}
        for split in splits:
            ...
            # Ensure x is in shape [batch_size, n_vars, seq_len]
            if len(x.shape) == 2:
                x = x.unsqueeze(0)  # Add batch dimension if needed
            if x.shape[1] != y.shape[1]:
                x = x.transpose(1, 2)  # Transpose if needed
            
            # Create dataset with proper windowing
            dataset = WindowDataset(x, y, self.window_size)
    ...
    def fit(self, data_dictionary: Dict[str, pd.DataFrame], splits: List[str]):
        """Training process for PatchTST model"""
        self.model.train()
        data_loaders_dictionary = self.create_data_loaders_dictionary(data_dictionary, splits)
        n_obs = len(data_dictionary["train_features"])
        n_epochs = self.n_epochs or self.calc_n_epochs(n_obs=n_obs)
        ...
        for epoch in range(n_epochs):
            ...
            for batch_data in data_loaders_dictionary["train"]:
                xb, yb = batch_data
                # Ensure correct shape [batch_size, n_vars, seq_len]
                if len(xb.shape) == 2:
                    xb = xb.unsqueeze(0)
                if xb.shape[1] != self.model.model.c_in:
                    xb = xb.transpose(1, 2)
