...
class RevIN(nn.Module):
    ...
    def _get_statistics(self, x):
        # Create a tuple by explicitly converting the range to a list first
        dim2reduce = tuple([i for i in range(1, x.ndim - 1)])
        if self.subtract_last:
            self.last = x[:, -1, :].unsqueeze(1).detach()
        else:
            self.sub = torch.mean(x, dim=dim2reduce, keepdim=True).detach()
